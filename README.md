...
- **🆕 Looksmaxxing Analysis** – Strict facial aesthetics evaluation.  
  ...
  - Objective **PSL rating** (1.0–8.0) and exact **tier** (Sub3 to True Adam / True Eve)
  ...
  - Stylish **infographic** (dark theme by default, light theme planned) with all metrics
...
```
*The updated `requirements.txt` now includes `Pillow>=10.0.0` for the infographic feature.*
...
| Command | Description |
|---------|-------------|
| `кульш [текст]` | General interaction – chat, ask questions, get roasted. |
| `кульш зайди в войс` | Summons the bot to your voice channel (starts listening with 65% response chance). |
| `кульш выйди` | Disconnects the bot from voice. |
| `кульш логи` | (Authorized only) Sends the current `bot.log` file to the chat. |
| **🆕 `кульш psl`** (or `кульш оценка`) | **Facial aesthetics analysis.**<br/>Send this command with an attached photo (or as a reply to a photo) to get a full PSL breakdown and infographic.<br/>Add `совет` or `advice` in the caption to receive practical looksmaxxing tips. Works in both Telegram and Discord. |
